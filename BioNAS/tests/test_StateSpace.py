...
class StateSpace_Test(unittest.TestCase):
	...
	def test_StateSpace(self):
		...
		state_space.add_layer(1, [State('maxpool1d', pool_size=10, strides=10), State('avgpool1d', pool_size=10, strides=10),])
	...
	def test_ModelBuild_from_StateSpace(self):
		state_space = StateSpace()
		state_space.add_layer(0, [State('conv1d', filters=3, kernel_size=10), State('conv1d', filters=10, kernel_size=10),])
		state_space.add_layer(1, [State('maxpool1d', pool_size=10, strides=10), State('avgpool1d', pool_size=10, strides=10),])
