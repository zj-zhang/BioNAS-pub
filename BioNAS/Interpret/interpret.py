...
def get_multiGPU_models_from_hist(hist_idx, hist, input_state, output_state, state_space, model_compile_dict):
	"""Given a set of indcies, build a dictionary of
	models from history file
	"""
	model_dict = {}
	for idx in hist_idx:
		model_state_str = [hist.iloc[idx]["L%i"%(i+1)] for i in range(hist.shape[1]-5)]
		model_dict[idx] = model_fn.build_multiGPU_sequential_model_from_string(model_state_str, input_state, output_state, state_space, model_compile_dict)
		model_dict[idx].load_weights('%s/weights/trial_%i/bestmodel.h5'%(hist.iloc[idx].dir, hist.iloc[idx].ID))
	return model_dict


def match_quantity(q, p, slice=10, num_sample_per_slice=None, replace=False, random_state=777):
	...
	if not num_sample_per_slice:
		num_sample_per_slice = int(np.ceil(len(q)/(100/slice)))
	q_quantiles = np.percentile(q, q=np.arange(0,100+slice, slice))
